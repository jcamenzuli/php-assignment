<?php
    ...
    function connect()
    {
        ...
        $link = mysqli_connect('localhost', 'root', '', 'db_phpassignment')
    ...
    function is_logged()
    {
        ...
        $auth       = (array_key_exists('auth_code', $_COOKIE)) ? $_COOKIE['auth_code'] : '';
    ...
    function register_login_data($email, $password, $salt)
    {
        ...
        // the sequence means: string string double integer double
        mysqli_stmt_bind_param($stmt, 'sss', $email, $password, $salt);
    ...
    function add_post($post)
    {
        ...
        // 3. Bind the parameters so we don't have to do the work ourselves.
        // the sequence means: string string double integer double
        mysqli_stmt_bind_param($stmt, 'isi', $date, $post, $id);
    ...
    function get_post(){
        ...
        $result = mysqli_query ($link, "
        SELECT a.*, b.name, b.surname
            FROM tbl_post a
            LEFT JOIN
               tbl_users_details b
            ON
               a.tbl_users_id = b.users_id
    ...
    function get_user($id)
    {
        ...
        // 2. Protext variables to avoid any SQL injection
        $id = mysqli_real_escape_string($link, $id);

        // 3. Generate a query and return result.
        $result = mysqli_query($link,"
        SELECT
            name AS 'user-name',
